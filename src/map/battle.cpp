...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case GN_CARTCANNON:
			...
			skillratio += -100 + (((250 * skill_lv) + ((20 * skill_lv) * (sd ? pc_checkskill(sd, GN_REMODELING_CART) : 1) + (sstatus->int_ * 2))) * status_get_lv(src)) / 100;
			RE_LVL_DMOD(100);
			break;
			...
			default: {
				...
				switch(skill_id) {
					...
					case WL_SOULEXPANSION:
						...
						skillratio += -100 + 1000 + skill_lv * 700;
					...
					case SO_PSYCHIC_WAVE:
						...
						skillratio *= 2; // TODO: IF BOOK/STAFF IS EQUIPPED
						break;
					case SO_CLOUD_KILL:
						skillratio += -100 + 40 * skill_lv;
						RE_LVL_DMOD(100);
